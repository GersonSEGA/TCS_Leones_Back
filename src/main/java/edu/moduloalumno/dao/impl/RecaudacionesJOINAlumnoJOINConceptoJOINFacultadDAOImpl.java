...
/*new*/@Override
    ...
    public List<RecaudacionesJOINAlumnoJOINConcepto> getRecaudacionesJOINAlumnoJOINConceptoByApeNom(String ape_nom) {
        String sql = "select a.ape_nom, c.concepto, r.fecha, r.id_rec, r.numero, r.id_alum, r.moneda, r.importe from recaudaciones r, alumno a, concepto c WHERE to_tsquery( translate( ? ,'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜ','aeiouAEIOUaeiouAEIOU') ) @@ to_tsvector(coalesce(translate( a.ape_nom ,'áéíóúÁÉÍÓÚäëïöüÄËÏÖÜ','aeiouAEIOUaeiouAEIOU'))) and a.id_alum = r.id_alum and c.id_concepto = r.id_concepto and a.id_alum not in (select id_alum from alumno_alumno_programa) order by r.fecha desc";
